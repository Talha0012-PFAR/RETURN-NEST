...
$count_stmt = $pdo->prepare("SELECT COUNT(*) FROM lost_items WHERE $where_clause");
$count_stmt->execute($params);
$total_items = $count_stmt->fetchColumn();
$total_pages = ceil($total_items / $per_page);

// Get items
$stmt = $pdo->prepare("SELECT li.*, u.name as user_name FROM lost_items li 
...
$categories = $pdo->query("SELECT DISTINCT category FROM lost_items WHERE status = 'approved' AND category IS NOT NULL")->fetchAll();
?>

<div class="d-flex justify-content-between align-items-center mb-4">
    <h2><i class="fas fa-exclamation-triangle text-danger"></i> Lost Items</h2>
    <?php if (isLoggedIn()): ?>
...
<!-- Results Count -->
<div class="mb-3">
    <p class="text-muted">Showing <?php echo $total_items; ?> lost item<?php echo $total_items != 1 ? 's' : ''; ?></p>
</div>
...
<?php else: ?>
                ...
                <div class="card h-100 shadow-sm">
                    ...
                    <div class="card-body">
                        ...
                        <div class="small text-muted mb-3">
                            ...
                            <div><i class="fas fa-calendar"></i> <?php echo date('M d, Y', strtotime($item['date_lost'])); ?></div>
                            <div><i class="fas fa-user"></i> <?php echo htmlspecialchars($item['user_name']); ?></div>
                        </div>
                        
                        <a href="view.php?id=<?php echo $item['id']; ?>" class="btn btn-outline-danger btn-sm">
    ...
    <?php if ($total_pages > 1): ?>
        <nav aria-label="Lost items pagination">
